...
2. 使用 unstructured 解析 docx，生成 _unstructured_sectionHeader_only.md 和 _unstructured_title_with_id.md
3. 使用 DocxXmlLoader + HeadingValidator 进行二次标题判定
...
def add_para_ids_to_docx(input_path: Union[str, Path], output_path: Optional[Union[str, Path]] = None) -> Path:
    """
...
def preprocess_docx_if_needed(docx_path: Union[str, Path], verbose: bool = True) -> Tuple[Path, bool]:
    """
...
class UnstructuredHeadingExtractor:
    ...
    def parse_docx(
        ...
        # 检测 DOCX 版本，如果是 Word 2007 则添加 paraId
